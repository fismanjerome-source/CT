...
-- Couvre directement la recherche de créneaux disponibles par centre (page
-- centre, recherche publique, "combler des horaires vides"...) : le filtre
-- porte systématiquement sur ces 4 colonnes ensemble. Sans cet index,
-- idx_creneaux_centre_date ne réduit que sur (centre_id, date), puis
-- statut/type_visite sont filtrés ligne par ligne après coup — coûteux
-- pour un centre avec beaucoup de créneaux au total (mesuré : ~2x plus
-- lent que pour un centre avec peu de créneaux, sur le même intervalle de
-- dates).
CREATE INDEX IF NOT EXISTS idx_creneaux_centre_statut_type_date ON creneaux(centre_id, statut, type_visite, date);
`;
...
async function runMigrations() {
  ...
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN note_interne TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN est_demo INTEGER NOT NULL DEFAULT 0`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN commission_taux_fixe REAL`);
  } catch {}
  try {
    await db.execute(`
  ...
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN created_at TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN code_parrainage TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN parraine_par_code TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN parrainage_recompense_le TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN premium_offert_jusqua TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN premium_offert INTEGER NOT NULL DEFAULT 0`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN est_premium INTEGER NOT NULL DEFAULT 0`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN premium_depuis TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE centres ADD COLUMN premium_desactive_le TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE controleurs ADD COLUMN telephone TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE controleurs ADD COLUMN created_at TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE controleurs ADD COLUMN email_details_envoye INTEGER DEFAULT 0`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE controleurs ADD COLUMN totp_secret TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE controleurs ADD COLUMN totp_actif INTEGER NOT NULL DEFAULT 0`);
  } catch {}
  try {
    // Pour les comptes déjà existants avant cette mise à jour (sans date de
    // création connue), on les marque comme déjà traités : seuls les
    // nouveaux comptes créés à partir de maintenant recevront l'email de
    // présentation détaillée 1h après leur inscription.
    await db.execute(`UPDATE controleurs SET email_details_envoye = 1 WHERE created_at IS NULL`);
  } catch {}
  try {
    await db.execute(`
      CREATE TABLE IF NOT EXISTS controleur_centres (
  ...
  try {
    await db.execute(`ALTER TABLE api_cles ADD COLUMN cle_hash TEXT`);
  } catch {}
  try {
    await db.execute(`ALTER TABLE api_cles ADD COLUMN cle_apercu TEXT`);
  } catch {}
  try {
    // Origine de la visite (ex: "avant-mon-ct") quand elle arrive avec un
    // paramètre utm_source connu — permet de distinguer, dans les
    // statistiques, les visites qui viennent d'Avant Mon CT du reste du
    // trafic client.
    await db.execute(`ALTER TABLE visites ADD COLUMN origine TEXT`);
  } catch {}
  ...
  // Migration sécurité : les clés API étaient stockées en clair (colonne
  // "cle"). On les bascule vers un hash (colonne "cle_hash"), sans jamais
  // invalider les clés déjà distribuées à un centre.
  try {
    const clesEnClair = await db.execute(
      `SELECT id, cle FROM api_cles WHERE cle_hash IS NULL AND cle IS NOT NULL`
    );
    for (const ligne of clesEnClair.rows) {
      const cle = ligne.cle;
      const hash = crypto.createHash('sha256').update(cle).digest('hex');
      const apercu = `${cle.slice(0, 12)}…${cle.slice(-4)}`;
      await db.execute({
        sql: `UPDATE api_cles SET cle_hash = ?, cle_apercu = ? WHERE id = ?`,
        args: [hash, apercu, ligne.id],
      });
    }
  } catch {
    // Pas grave si ça échoue (ex: table pas encore créée au tout premier lancement).
  }
  ...
  // Génère un code de parrainage pour tout centre qui n'en a pas encore
  // (centres créés avant cette fonctionnalité) — en direct avec db.execute,
  // jamais via get()/run(), pour éviter tout risque de blocage circulaire
  // avec ensureSchema().
  try {
    const CARACTERES_CODE = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    const sansCode = await db.execute('SELECT id FROM centres WHERE code_parrainage IS NULL');
    for (const ligne of sansCode.rows) {
      let code;
      let unique = false;
      for (let tentative = 0; tentative < 10 && !unique; tentative++) {
        code = '';
        for (let i = 0; i < 6; i++) code += CARACTERES_CODE[Math.floor(Math.random() * CARACTERES_CODE.length)];
        const collision = await db.execute({ sql: 'SELECT id FROM centres WHERE code_parrainage = ?', args: [code] });
        unique = collision.rows.length === 0;
      }
      await db.execute({ sql: 'UPDATE centres SET code_parrainage = ? WHERE id = ?', args: [code, ligne.id] });
    }
  } catch {
    // Pas grave si ça échoue — les codes manquants seront générés au prochain démarrage.
  }
...
Le Centre peut, à sa seule initiative et sans engagement, souscrire au statut optionnel « Premium » (30 € TTC par mois), qui lui permet d'apparaître en tête des résultats de recherche avec un signe distinctif visible par les Utilisateurs. Ce statut est facturé au prorata du nombre de jours d'activation le premier mois, selon les mêmes modalités et échéances que la commission (article 7), et peut être arrêté à tout moment par le Centre depuis son espace professionnel, le mois de l'arrêt étant lui aussi calculé au prorata.
