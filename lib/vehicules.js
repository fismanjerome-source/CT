...
export const TYPES_VEHICULES = [
  ...
  // Catégorie "pl" (poids lourd) : sert de base au sous-domaine
  // pl.creneauct.fr, qui ne propose que ces types-là. Un centre qui les
  // coche apparaît automatiquement sur ce sous-domaine (voir
  // /api/centres, filtre "categorie").
  { value: 'camion', label: 'Camion / poids lourd', categorie: 'pl', icone: 'camion', couleur: '#4A5859' },
  { value: 'autocar', label: 'Autocar / Autobus', categorie: 'pl', icone: 'autocar', couleur: '#3B6E8F' },
];

// Catégories "légères" (voiture + moto), regroupées pour le site principal
// creneauct.fr, par opposition à la catégorie "pl" du sous-domaine dédié.
export const CATEGORIES_VL = ['voiture', 'moto'];

export function libelleType(value) {
  return TYPES_VEHICULES.find((t) => t.value === value)?.label || value;
}

// Types de véhicules à proposer selon le site consulté (creneauct.fr pour
// les véhicules légers, pl.creneauct.fr pour les poids lourds) — utilisé
// pour le sélecteur "Mon véhicule" et pour scoper la recherche de centres.
export function typesParSite(site) {
  return TYPES_VEHICULES.filter((t) =>
    site === 'pl' ? t.categorie === 'pl' : CATEGORIES_VL.includes(t.categorie)
  );
}
