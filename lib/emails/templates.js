...
// Les noms, emails, immatriculations... viennent tous en dernier ressort
// d'un champ texte libre rempli par un client anonyme (réservation) ou un
// centre auto-inscrit (aucune vérification à l'inscription) — jamais
// validés contre l'injection de HTML. Sans échappement, une simple balise
// dans un prénom ("Jean<b>") atterrirait telle quelle dans l'email HTML
// envoyé à quelqu'un d'autre (le centre, un autre client, l'admin).
...
export function emailDetailsEspacePro({ nom, nomCentre }) {
  ...
  nomCentre = echapperHtml(nomCentre);
  return {
    ...
    html: enveloppe(`
      ...
      <ul style="color:#1A2420; line-height:1.8; padding-left:20px;">
        ...
        <li>Connectez votre logiciel de planning grâce à une clé API personnelle</li>
      ...
      <p style="color:#1A2420; line-height:1.6; background:#E5F1E7; border-radius:6px; padding:12px 16px;">
        🔑 <strong>Déjà un logiciel de planning ?</strong> Générez une clé API personnelle dans l'onglet « Clés API »
        de votre espace : elle donne à votre logiciel un accès en lecture à vos créneaux et vos rendez-vous, pour
        éviter toute ressaisie manuelle. Un simple lien d'abonnement d'agenda (à coller dans Google Calendar,
        Outlook...) est aussi disponible au même endroit.
      </p>
      <p style="color:#1A2420; line-height:1.6;">
...
export function emailRappelCommissionMensuelle({ nomControleur, nomCentre, mois, montant, dateLimite }) {
  nomControleur = echapperHtml(nomControleur);
  nomCentre = echapperHtml(nomCentre);
  return {
...
export function emailChangementRdvCentre({ nomControleur, type, clientNom, dateLisible, heure, ancienneDateLisible, ancienneHeure, reference }) {
  nomControleur = echapperHtml(nomControleur);
  clientNom = echapperHtml(clientNom);
...
export function emailNouvelleReservationCentre({ nomControleur, clientNom, clientTelephone, clientEmail, dateLisible, heure, typeVehiculeLabel, immatriculation, reference, prixPaye }) {
  nomControleur = echapperHtml(nomControleur);
  clientNom = echapperHtml(clientNom);
  clientTelephone = echapperHtml(clientTelephone);
  clientEmail = echapperHtml(clientEmail);
  typeVehiculeLabel = echapperHtml(typeVehiculeLabel);
  immatriculation = echapperHtml(immatriculation);
  return {
...
export function emailConfirmationReservation({ clientNom, centreNom, adresse, dateLisible, heure, reference, typeVehiculeLabel }) {
  clientNom = echapperHtml(clientNom);
  centreNom = echapperHtml(centreNom);
  adresse = echapperHtml(adresse);
  typeVehiculeLabel = echapperHtml(typeVehiculeLabel);
  return {
