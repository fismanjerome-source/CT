...
export async function facturesEnRetard(centreId) {
  ...
  const centre = await get('SELECT est_premium, premium_depuis, premium_desactive_le FROM centres WHERE id = ?', [centreId]);
...
export async function commissionMoisEnCours(centreId) {
  ...
  const centre = await get('SELECT premium_depuis, premium_desactive_le FROM centres WHERE id = ?', [centreId]);
