...
//
// Le formatteur Intl est construit une seule fois et réutilisé : le
// reconstruire à chaque appel coûte ~18x plus cher (mesuré : 0,35ms contre
// 0,02ms par appel) — négligeable à l'unité, mais creneauSuffisammentEloigne
// est appelée une fois par créneau lors de l'affichage d'un calendrier de
// centre, donc ça devenait un ralentissement réel (et variable selon le
// nombre de créneaux ouverts à ce moment-là) sur un centre avec beaucoup de
// créneaux.
const formatteurDecalageParis = new Intl.DateTimeFormat('en-US', {
  timeZone: 'Europe/Paris', hour: 'numeric', hour12: false, timeZoneName: 'shortOffset',
});
function decalageParisHeures(instant) {
  const partie = formatteurDecalageParis.formatToParts(instant).find((p) => p.type === 'timeZoneName')?.value || 'GMT+1';
...
// 25% si le RDV est dans les 7 jours, 20% entre 7 et 14 jours, 15% au-delà.
...
export function calculerTauxCommission(dateCreneauStr) {
  ...
  if (joursDiff <= 7) return 25;
  if (joursDiff <= 14) return 20;
  return 15;
}
